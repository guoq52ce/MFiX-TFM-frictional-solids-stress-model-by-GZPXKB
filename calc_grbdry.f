...
!!qg            ( ( N_Pc*((ONE-ep_star-delta-eps_f_min)**(N_Pc-ONE))/(delta**D_Pc)) + &
!!qg              ( D_Pc*((ONE-ep_star-delta-eps_f_min)**N_pc)/(delta**(D_pc+ONE)) ) )
!!qg         Pc_at = (to_SI*Fr)*&
!!qg           ((ONE-ep_star-delta-EPS_f_min)**N_Pc) / (delta**D_Pc)
!!qg         Pc = Pc_at + dPcdep_at*(EPg-ep_star-delta)
!		 Pc = 2d0*SddS*(b_dilation**2)*(Dp(M)**2)*ros(M) / &
!					 ( delta**2) + 4d0*SddS*(b_dilation**2)*(Dp(M)**2)*ros(M) / &
!					 ( delta**3) * ((ONE-EPG)+delta-(ONE-ep_star))
		 Pc = 2d0*SddS*(b_dilation**2)*(Dp(M)**2)*ros(M) / &
					 ( delta**2) + to_SI*a_ps * ((ONE-EPG)-(ONE-ep_star)+delta)**b_ps
      ELSE
!!qg         Pc = (to_SI*Fr)*((ONE-EPg-EPS_f_min)**N_Pc) / &
!!qg            ( ((ONE-ep_star)-(ONE-EPg) )**D_Pc)
		 Pc = 2d0*SddS*(b_dilation**2)*(Dp(M)**2)*ros(M) / &
