...
!---------------------------------------------------------------------
      ...
      USE run, only: friction, savage, blending_stress
...
! maximum value of solids viscosity in poise
	  ...
	  DOUBLE PRECISION :: b_dilation
	  DOUBLE PRECISION :: yita_dian
	  DOUBLE PRECISION :: n1_t
	  DOUBLE PRECISION :: n2_t
      PARAMETER (MAX_MU_s = 1000.D0)	  
	  PARAMETER (b_dilation = 0.13)
...
!********************************************qg***********************************************
					 yita_dian=SQRT(2d0*(trD_s2(IJK,M) - ((trD_s_C(IJK,M)*trD_s_C(IJK,M))/3.d0)))
...
!!qg                        dPcdep_at = -(to_SI*Fr)*&
!!qg                           ( ( N_Pc*((ONE-ep_star_array(IJK)-&
!!qg                           delta-eps_f_min)**(N_Pc-ONE))/(delta**D_Pc)) + &
!!qg                           ( D_Pc*((ONE-ep_star_array(ijk)-&
!!qg                           delta-eps_f_min)**N_pc)/(delta**(D_pc+ONE)) ) )
!!qg                        Pc_at = (to_SI*Fr)*&
!!qg                          ((ONE-ep_star_array(IJK)-delta-EPS_f_min)**N_Pc) / (delta**D_Pc)
!!qg                        Pc = Pc_at + dPcdep_at*(EP_G(IJK)-ep_star_array(IJK)-delta)
!!qg                        dPcdeps = -dPcdep_at
!						Pc = (yita_dian**2)*(b_dilation**2)*(D_p(IJK,M)**2)*ro_s(IJK,M) / &
!							( delta**2) + 2*(yita_dian**2)*(b_dilation**2)*(D_p(IJK,M)**2)*ro_s(IJK,M) / &
!							( delta**3) * ((ONE-EP_G(IJK))+delta-(ONE-ep_star_array(ijk)))
!						dPcdeps = 2*(yita_dian**2)*(b_dilation**2)*(D_p(IJK,M)**2)*ro_s(IJK,M)/( delta**3)
						Pc = (yita_dian**2)*(b_dilation**2)*(D_p(IJK,M)**2)*ro_s(IJK,M) / &
                     ...
                     ELSE
!!qg                        Pc = (to_SI*Fr)*((ONE-EP_G(IJK)-EPS_f_min)**N_Pc) / &
!!qg                           ( ((ONE-ep_star_array(ijk)) - (ONE-EP_G(IJK)))**D_Pc)
!!qg                        dPcdeps = (to_SI*Fr)*&
!!qg                           ( ( N_Pc*((ONE-EP_G(IJK)-eps_f_min)**(N_Pc-ONE))/&
!!qg                           ((EP_G(IJK)-ep_star_array(ijk))**D_Pc) ) + &
!!qg                           ( D_Pc*((ONE-EP_G(IJK)-eps_f_min)**N_pc)/&
!!qg                           ((EP_G(IJK)-ep_star_array(IJK))**(D_Pc+ONE)) ) )
						Pc = (yita_dian**2)*(b_dilation**2)*(D_p(IJK,M)**2)*ro_s(IJK,M) / &
							( ((ONE-ep_star_array(ijk))-(ONE-EP_G(IJK)) )**2)*n1_t
                        dPcdeps = (20000*b_dilation**2*D_p(IJK,M)**2*ro_s(IJK,M)*yita_dian**2)/ &
						             (pi*((ONE-EP_G(IJK)) - (ONE-ep_star_array(ijk)))**2*((10000*(ONE-EP_G(IJK)) - 10000*EPS_f_min)**2 + 1)) - &
									 (4*b_dilation**2*D_p(IJK,M)**2*ro_s(IJK,M)*yita_dian**2*atan(10000*(ONE-EP_G(IJK)) - 10000*EPS_f_min))/ &
